...
describe('FallbackClient — mixed finality (finalized-only bulk, hot tip)', () => {
  /** Inclusive block range — the stream starts at `QUERY.fromBlock` (0). */
  const range = (from: number, to: number) => Array.from({ length: to - from + 1 }, (_, i) => from + i)

  /** A finalized-only source: serves up to its finalized head, then waits for finality to advance. */
  function finalizedBulk(name: string, finalizedHead: number, chainTip = finalizedHead) {
    const s = source(
      name,
      async function* (q: any) {
        for (let n = q.fromBlock ?? 1; n <= finalizedHead; n++) yield batch(n)
        await hang() // above its finalized head there is nothing to serve *yet*
      },
      // Answers honestly per commitment, like a real portal: its finalized head, or the chain tip
      // when explicitly asked for the unfinalized one.
      async (options?: { finalized: boolean }) => cursor(options?.finalized === false ? chainTip : finalizedHead),
    )
    ;(s.client as any).finalized = true
  ...
  /** A hot source that follows the chain tip. */
  function hotTip(name: string, tip: number) {
    return source(
      name,
      async function* (q: any) {
        for (let n = q.fromBlock ?? 1; n <= tip; n++) yield batch(n)
      },
      async () => cursor(tip),
    )
  }

  it('hands off at the finality frontier and resumes cursor-continuously', async () => {
    const bulk = finalizedBulk('bulk', 100)
    const tip = hotTip('tip', 105)
    const fb = fallback([bulk, tip], { maxStalenessMs: 40, freshnessTickMs: 10, headTtlMs: 0, maxLagBlocks: null })

    expect(await collect(fb)).toEqual(range(0, 105))
    // One switch, and the hot source was asked to continue from just past the finalized head.
    expect(fb.switchCount).toBe(1)
    expect(fb.activeIndex).toBe(1)
    expect(tip.reads[0]).toMatchObject({ fromBlock: 101, parentBlockHash: '0x100' })
  }, 10_000)

  it('is driven by staleness, not lag: lag alone cannot rescue a source that stopped delivering', async () => {
    // Lag is evaluated at batch boundaries, and a source parked at its finalized head delivers no
    // further batch — so with staleness disabled the pipe would wait forever. This pins the reason
    // `maxStalenessMs` (not `maxLagBlocks`) is the knob for this topology.
    const fb = fallback([finalizedBulk('bulk', 100), hotTip('tip', 105)], {
      maxStalenessMs: null,
      maxLagBlocks: 2,
      freshnessTickMs: 10,
      headTtlMs: 0,
    ...
    const outcome = await Promise.race([
      ...
      wait(300).then(() => 'still waiting' as const),
    ])

    expect(outcome).toBe('still waiting')
    expect(got[got.length - 1]).toBe(100) // parked at the frontier, still on the bulk source
    expect(fb.activeIndex).toBe(0)
  }, 10_000)

  it('polls each source at its own commitment, so an exhausted source never looks "fresher"', async () => {
    // The bulk source's reach is its FINALIZED head (100) even though the chain is at 200. Polling
    // it at the *pipe's* commitment would report 200 — making a source that cannot serve block 151
    // look ahead of a hot active source stalled at 150, and inviting a switch straight into a
    // second stall.
    const tip = source('tip', async function* (q: any) {
      for (let n = q.fromBlock ?? 1; n <= 150; n++) yield batch(n)
      await hang() // the hot source stops delivering
    })
    const bulk = finalizedBulk('bulk', 100, 200)
    const fb = fallback([tip, bulk], { maxStalenessMs: 40, freshnessTickMs: 10, headTtlMs: 0, maxLagBlocks: null })
    ...
    expect(got[got.length - 1]).toBe(150)
    expect(fb.activeIndex).toBe(0) // held: nothing is actually ahead of us
    expect(fb.switchCount).toBe(0) // no switch into a source that cannot serve block 151
    expect(fb.chainStalled).toBe(true) // and it is reported as a chain stall, which is what it is
    expect(bulk.reads).toHaveLength(0)
  }, 10_000)

  it('does not crawl back to the exhausted source once past its frontier', async () => {
    // After the handoff the pipe is above the bulk source's finalized head, so eager switch-up must
    // not reclaim it. Its capability probe fails at the frontier, which is what holds the line.
    const bulk = finalizedBulk('bulk', 100)
    const tip = source(
      'tip',
      async function* (q: any) {
        for (let n = q.fromBlock ?? 1; n <= 130; n++) {
          yield batch(n)
          await wait(2)
        }
      },
    ...
    const fb = fallback(
      [bulk, tip],
      {
        ...
        cooldownMs: 20,
        livenessRecoverThreshold: 1,
        capabilityProbe: { timeoutMs: 30 },
        capabilityProbeIntervalMs: 0,
      },
      {},
    )

    expect(await collect(fb)).toEqual(range(0, 130))
    expect(fb.activeIndex).toBe(1) // finished on the hot source
    expect(fb.switchCount).toBe(1) // handed off exactly once — no thrash back to the bulk source
  }, 15_000)
})

...
Last updated: 2026-08-27. Statuses reflect the actual TypeScript test inventory at this
change's mainline merge base plus the control-route suite in this change, not aspiration.
...
| RP-45 (control channel) | CT-2/5 | P ⚠ | one announcement per fork, its CDC envelope and default id, its enqueue ahead of the compensations it precedes, the epoch reaching data operations, and its exclusion from the blast-radius histogram are asserted against a publisher seam; the crash *between* the fork transaction and its drain is covered only by the same commit-then-publish argument as CN-17, with no kill-point test of its own (GAP-14's family) |
| RP-46 (version sequence barrier) | CT-5 | P ⚠ | the gap-free run on a shared destination is asserted for both control record kinds, and compensations are asserted to sequence above the announcement that precedes them; block order and the multi-destination and filtered-subscription voids are stated in the contract but not exercised — no test drives a two-topic producer or a filtered subscriber |
| RP-47 (finality watermark) | CT-5 | P ⚠ | the published value, its default id, its attribution to the committed block, the `everyBlocks` throttle and its advance while the routed stream yields no rows are asserted against a publisher seam; that a consumer cannot read it as a per-row proof is a documentation obligation with no test |
...
| GAP-39 | Class C states finality as a reference value only (RP-47): a consumer learns the source's finalized head but never that a given row is reorg-proof, and the two differ wherever the source's finality policy differs from the consumer's. This is a decision, not an omission (ADR-23) — the residue is that a binding could still word the watermark as a proof | CN-17, RP-24, RP-47 | P3 | consumer asks "is this row final?" → assert the contract answers with the source's view and its policy caveat, never with a per-row verdict |
